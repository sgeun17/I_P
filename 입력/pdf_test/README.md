...
| `test_pdf_parser.py` | 팀 규칙 + 정상/예외 테스트 36개 (samples 폴더 사용 → **지우지 마세요**) |
| `samples/` | 테스트용 PDF 14개 |
...
python test_pdf_parser.py                    # "✅ 36개 통과, ❌ 0개 실패" 나오면 정상
...
| 14 머리글·바닥글 4쪽 | 1쪽 머리글·바닥글만 남고 2~4쪽 반복은 버려짐, 본문 4개는 그대로 |
...
## 머리글·바닥글 (매 페이지 반복되는 줄)

문서명·회사명·쪽 번호처럼 페이지마다 반복되는 줄은 **맨 처음 나온 것 하나만 남기고, 그 뒤 반복은 버려요** (**팀 규칙에 추가 필요**).
머리글의 "| 대외비" 같은 보안등급 표시는 그 자체가 증적이 될 수 있어서 문서에 한 번은 남겨 둡니다.
판단 기준은 두 가지예요.
1. 페이지 맨 위 또는 맨 아래 `EDGE_ZONE_RATIO`(12%) 안에 있는 글자
2. 같은 글이 3쪽 이상 + 전체 페이지의 절반 이상에서 반복 (쪽 번호만 다른 것도 잡으려고 숫자는 무시하고 비교)

표는 절대 버리지 않고, 조건이 안 맞으면 아무것도 바꾸지 않아요. 2쪽짜리 짧은 문서도 그대로 둬요.
가운데(본문)에 있는 제목은 머리글과 글자가 비슷해도 남아요.
...
- 3쪽 미만 문서나 페이지 절반 미만에만 나오는 머리글·바닥글은 그대로 남아요 (예: 엑셀 PDF 의 인쇄 제목).
